...
export async function getSystemInfo() {
  const [ramOut, diskOut, uptimeOut, loadOut, cpuOut, servicesOut, dockerOut] = await Promise.all([
    ...
    runQuick('nproc'),
    runQuick('systemctl is-active nginx docker mariadb php8.2-fpm fail2ban admin-dashboard postfix dovecot rspamd redis-server 2>/dev/null'),
  ...
  // Parse Services
  const serviceNames = ['nginx', 'docker', 'mariadb', 'php8.2-fpm', 'fail2ban', 'admin-dashboard', 'postfix', 'dovecot', 'rspamd', 'redis-server']
