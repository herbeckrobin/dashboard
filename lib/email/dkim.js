...
export function getDnsRecords(domain) {
  ...
  const { mailDomain } = getDomainConfig()
  const records = []
  ...
  records.push({
    type: 'TXT',
    host: domain,
    value: `v=spf1 mx a:${mailDomain} ~all`,
  ...
  records.push({
    type: 'TXT',
    host: `_dmarc.${domain}`,
    value: `v=DMARC1; p=quarantine; rua=mailto:postmaster@${getDomainConfig().serverDomain}; pct=100`,
