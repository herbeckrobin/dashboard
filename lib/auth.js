...
const RP_ID = 'admin.rhdemo.de'
const RP_ORIGIN = 'https://admin.rhdemo.de'
...
export function setSessionCookie(res, jwt) {
  res.setHeader('Set-Cookie', `session=${jwt}; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age=${30 * 24 * 60 * 60}`)
}
...
// Store current challenge temporarily (in-memory, per-process)
