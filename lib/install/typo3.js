...
export function getTypo3Steps(project) {
  ...
  steps.push({
    name: 'TYPO3 Setup',
    cmd: `cd ${projectPath} && TYPO3_DB_DRIVER=mysqli TYPO3_DB_HOST=${database.host} TYPO3_DB_PORT=3306 TYPO3_DB_DBNAME=${database.name} TYPO3_DB_USERNAME=${database.user} TYPO3_DB_PASSWORD="${database.password}" TYPO3_SETUP_ADMIN_USERNAME=admin TYPO3_SETUP_ADMIN_PASSWORD="${adminPassword}" TYPO3_SETUP_ADMIN_EMAIL=${getAdminEmail()} TYPO3_SETUP_CREATE_SITE="https://${domain}" TYPO3_PROJECT_NAME="${name}" TYPO3_SERVER_TYPE=other vendor/bin/typo3 setup --force --no-interaction`
  })
