...
`)
  ...
  steps.push({
    ...
    cmd: `cd ${projectPath} && find node_modules -xtype l -delete 2>/dev/null; git init -b main && git config user.name "Deploy Dashboard" && git config user.email "deploy@rhdemo.de" && git add . && git commit -m "Next.js Starter"`
  })

  // 5. Git Push
  steps.push({
    name: 'Git Push (Next.js)',
    cmd: `cd ${projectPath} && git remote add origin ${gitUrl} && git push -u origin main --force`
  })
