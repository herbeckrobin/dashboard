...
export function getContaoSteps(project) {
  ...
  steps.push({
    ...
    cmd: `cd ${projectPath} && mysql --default-character-set=utf8mb4 -u ${escapeShellArg(database.user)} -p${escapeShellArg(database.password)} ${escapeShellArg(database.name)} < setup-content.sql && rm setup-content.sql && echo "Content eingefuegt"`
  })
