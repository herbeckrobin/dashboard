...
import { randomPassword, getGitUrl, getGitConf, getGitCredentialHelper } from './shared'
...
export function getContaoSteps(project) {
  ...
  steps.push({
    ...
    cmd: `cd ${projectPath} && echo "de" | vendor/bin/contao-console contao:user:create --username=admin --name=Admin --email=${escapeShellArg(getAdminEmail())} --password=${escapeShellArg(adminPassword)} --admin`
  })
  ...
  steps.push({
    ...
    cmd: `cd ${projectPath} && git init -b main && git add -A && git ${getGitConf()} commit -m "Contao Installation" && git remote add origin ${gitUrl} && git ${getGitCredentialHelper()} push -u origin main --force`
  })
