...
export function getContaoSteps(project) {
  ...
  steps.push({
    ...
    cmd: `cd ${projectPath} && echo "de" | vendor/bin/contao-console contao:user:create --username=admin --name=Admin --email=info@robinherbeck.com --password="${adminPassword}" --admin`
  })
  ...
  if (aiStep) {
    ...
    steps.push({
      ...
      run: async () => {
        ...
        const sql = generateContentSQL(siteData.sections, name, googleFontsUrl, 'info@robinherbeck.com')
  ...
  steps.push({
    ...
    cmd: `cd ${projectPath} && git init -b main && git add -A && git ${GIT_CONF} commit -m "Contao Installation" && git remote add origin ${gitUrl} && git push -u origin main --force`
  })
