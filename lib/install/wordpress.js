...
export function getWordPressSteps(project) {
  ...
  steps.push({
    ...
    cmd: `wp config create --path=${projectPath} --dbname=${database.name} --dbuser=${database.user} --dbpass=${database.password} --dbhost=${database.host}`
  })

  // 4. WP installieren
  steps.push({
    name: 'WordPress installieren',
    cmd: `wp core install --path=${projectPath} --url=https://${domain} --title="${name}" --admin_user=admin --admin_password="${adminPassword}" --admin_email=info@robinherbeck.com --skip-email`
  })
