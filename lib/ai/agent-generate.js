...
async function runAgentSDK(prompt, cwd, onOutput) {
  ...
  try {
    ...
    // tool_use_id -> { toolName, filePath } Mapping fuer Result-Zuordnung
    ...
    for await (const message of query({
      ...
      if (message.type === 'assistant' && content.length > 0) {
        for (const block of content) {
          if (block.type === 'text' && block.text) {
            ...
            if (toolName === 'Write' || toolName === 'Edit') {
              ...
              if (onOutput) onOutput(`  [${turnCount}] ${toolName}: ${filePath}\n`)
              // Merke Write/Edit fuer Result-Zuordnung
              if (block.id) {
                pendingWrites.set(block.id, { toolName, filePath })
              }
        ...
        // Tool-Ergebnisse auswerten (Erfolg/Fehler der Write/Edit-Aufrufe)
        for (const block of content) {
          if (block.type === 'tool_result' && pendingWrites.has(block.tool_use_id)) {
            const { toolName, filePath } = pendingWrites.get(block.tool_use_id)
            ...
            const resultStr = resultContent.filter(c => c.type === 'text').map(c => c.text).join('')
            const isError = block.is_error || resultStr.toLowerCase().includes('error') || resultStr.toLowerCase().includes('failed')
            if (isError) {
              writeErrorCount++
              const shortError = resultStr.slice(0, 150)
              if (onOutput) onOutput(`  [FEHLER] ${toolName} ${filePath}: ${shortError}\n`)
              console.log(`[Agent] Write-Fehler: ${filePath}: ${shortError}`)
            } else {
    ...
    const writeSuccessCount = writtenFiles.size
    console.log(`[Agent] Fertig: ${turnCount} Tool-Aufrufe, ${writeCount} Write-Versuche, ${writeSuccessCount} erfolgreich, ${writeErrorCount} Fehler`)
    if (onOutput) onOutput(`→ Agent fertig: ${turnCount} Tool-Aufrufe, ${writeSuccessCount}/${writeCount} Writes erfolgreich`)
    if (writeErrorCount > 0) {
      if (onOutput) onOutput(` (${writeErrorCount} Fehler)`)
    }
    if (onOutput) onOutput(`\n`)

    return { success: true, resultText, turnCount, writeCount, writeSuccessCount, writeErrorCount, writtenFiles: [...writtenFiles], costUsd }
  } catch (err) {
...
export async function generateWithAgent(project, projectPath, designBrief, onOutput) {
  ...
  const { resultText, writeCount, writeSuccessCount = 0, writeErrorCount = 0, costUsd } = result

  if (writeSuccessCount === 0 && writeCount === 0) {
  ...
  if (writeSuccessCount === 0 && writeCount > 0) {
    if (onOutput) onOutput(`→ Agent hat ${writeCount} Writes versucht, aber alle sind fehlgeschlagen\n`)
    return { success: false, costUsd, error: `Alle ${writeCount} Write-Versuche fehlgeschlagen` }
  }
  ...
  try {
    ...
    if (pageContent.includes('Next.js Starter') && writeSuccessCount < 3) {
...
ARBEITSVERZEICHNIS: ${themePath}
WICHTIG: Alle Dateipfade muessen ABSOLUT sein! Nutze immer ${themePath}/ als Prefix.

BEREITS VORHANDEN (NICHT aendern!):
- ${themePath}/functions.php (registriert Blocks aus build/blocks/ automatisch)
- ${themePath}/package.json (wp-scripts installiert, node_modules vorhanden)
- ${themePath}/templates/ und ${themePath}/parts/ (Basis-Templates)

ERSTELLE FOLGENDE DATEIEN:

1. ${themePath}/theme.json — Farben, Fonts, Spacing passend zur Beschreibung.
   Definiere CSS Custom Properties via settings.custom (z.B. "primary", "secondary", "bg", "text").

2. ${themePath}/style.css — NUR Theme-Header + Google Font @import + minimaler Reset (box-sizing, margin:0).
   KEINE Block-Styles hier! Alle visuellen Styles gehoeren in die Block-SCSS-Dateien.

3. Mindestens 5 Blocks in ${themePath}/src/blocks/{name}/:

   Jeder Block braucht 3 Dateien:

   ${themePath}/src/blocks/{name}/block.json:
   ...
   ${themePath}/src/blocks/{name}/index.js:
   ...
   ${themePath}/src/blocks/{name}/style.scss:
...
4. PHP Render-Callback fuer jeden Block:
   ${themePath}/includes/render-{name}.php
...
5. ${themePath}/_frontpage.html mit Block-Markup:
...
export async function generateWithAgentWP(project, themePath, designBrief, onOutput) {
  ...
  const { resultText, writeCount, writeSuccessCount = 0, writeErrorCount = 0, writtenFiles = [], costUsd } = result
  ...
  if (writeSuccessCount === 0 && writeCount > 0) {
    if (onOutput) onOutput(`→ Agent hat ${writeCount} Writes versucht, aber alle sind fehlgeschlagen\n`)
    if (writtenFiles.length === 0 && onOutput) {
      onOutput(`→ Keine Dateien auf dem Dateisystem erstellt\n`)
    }
    return { success: false, costUsd, error: `Alle ${writeCount} Write-Versuche fehlgeschlagen` }
  }
  ...
  try {
    ...
    console.log(`[Agent WP] Blocks: ${srcBlocks} in src/blocks/, ${buildBlocks} in build/blocks/, ${writeSuccessCount}/${writeCount} Writes OK, ${writeErrorCount} Fehler`)

    if (srcBlocks === 0 && buildBlocks === 0) {
      // Zeige wohin der Agent tatsaechlich geschrieben hat
      const blockFiles = writtenFiles.filter(f => f.includes('/src/blocks/') || f.includes('/build/blocks/'))
      const otherFiles = writtenFiles.filter(f => !f.includes('/src/blocks/') && !f.includes('/build/blocks/'))
      if (onOutput) {
        onOutput(`→ Agent hat keine Blocks in src/blocks/ erstellt\n`)
        if (blockFiles.length > 0) {
          onOutput(`→ Block-Dateien geschrieben (aber keine Unterordner): ${blockFiles.join(', ')}\n`)
        }
        if (otherFiles.length > 0) {
          onOutput(`→ Dateien ausserhalb von blocks/: ${otherFiles.slice(0, 10).join(', ')}${otherFiles.length > 10 ? ` (+${otherFiles.length - 10} weitere)` : ''}\n`)
        }
        if (writeErrorCount > 0) {
          onOutput(`→ ${writeErrorCount} Write-Fehler aufgetreten (siehe Logs oben)\n`)
        }
      }
      return { success: false, costUsd, error: `Agent hat keine Blocks erstellt (${writeSuccessCount} Dateien geschrieben, ${writeErrorCount} Fehler)` }
    }
  } catch (e) {
