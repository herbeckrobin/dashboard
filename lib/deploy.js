...
export function generateNginxConfig(domain, port, type, projectPath, project = {}) {
  ...
  const cspExtra = (project.cspWhitelist || []).map(d => ` https://${d.replace(/^\*\./, '*.').trim()}`).join('')
  const securityHeaders = `
    include snippets/security-headers.conf;
    add_header Content-Security-Policy "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'${cspExtra}; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; font-src 'self' data:; connect-src 'self'${cspExtra}; frame-src 'self' blob:; worker-src 'self' blob:; frame-ancestors 'self';" always;`
