...
import { initLog, updateStep, finishLog } from './deploy-log'
...
export async function deployProject(project, { trigger = 'manual', triggerInfo = null } = {}) {
  ...
  // Git URL mit Token fuer private Repos
  ...
  steps.push({
    name: 'Git Clone / Pull',
    cmd: `if [ -d "${gitPath}/.git" ]; then cd ${gitPath} && git fetch origin && BRANCH=$(git symbolic-ref --short HEAD 2>/dev/null || echo main) && if git rev-parse --verify origin/$BRANCH >/dev/null 2>&1; then git reset --hard origin/$BRANCH; else echo "Remote branch origin/$BRANCH existiert noch nicht - uebersprungen"; fi; else git clone ${gitUrl} ${gitPath}; fi`
  })
  ...
  if (project.previousDomain && project.previousDomain !== domain) {
    ...
    steps.push({
      ...
      cmd: `sudo rm -f /etc/nginx/sites-enabled/${oldDomain} /etc/nginx/sites-available/${oldDomain} && sudo nginx -t && sudo systemctl reload nginx`
    })
  }
  ...
  const isCustomDomain = !domain.endsWith('.rhdemo.de')
  if (isCustomDomain) {
    try {
    ...
    wwwDnsReady = true // rhdemo.de Subdomains brauchen keinen DNS-Check
  }
  ...
  if (mainDnsReady) {
    ...
    steps.push({
      name: 'SSL Zertifikat',
      cmd: `sudo certbot --nginx --expand ${certDomains} --non-interactive --agree-tos --email info@robinherbeck.com || true`
    })
  ...
  apps: [{
    ...
    env: ${JSON.stringify(envObj, null, 6)}
      ...
      cmd: `cd ${projectPath} && pm2 delete ${name} 2>/dev/null; pm2 start ecosystem.config.js && pm2 save`
    })
  }
  ...
  if (token && repo) {
    steps.push({
      ...
      run: async () => {
        const webhookUrl = `http://172.18.0.1:9000/deploy/${name}`
        const webhookSecret = '9cc082e3bb8082d85b8e04eff9ad9fed33a21438a4a190fcab0e7cdddbdc7e52'
        const giteaApi = `http://localhost:3000/api/v1/repos/${repo}/hooks?token=${encodeURIComponent(token)}`

        // Bestehende Hooks abrufen
        const existing = await fetch(giteaApi)
        const hooks = await existing.json()

        if (Array.isArray(hooks)) {
          // Bestehenden Webhook auf gleiches Projekt finden und aktualisieren
          for (const hook of hooks) {
            if (hook.config?.url?.includes('/deploy/')) {
              await fetch(`http://localhost:3000/api/v1/repos/${repo}/hooks/${hook.id}?token=${encodeURIComponent(token)}`, {
                method: 'PATCH',
