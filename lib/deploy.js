...
export function generateNginxConfig(domain, port, type, projectPath, project = {}) {
  ...
  const cspExtra = (project.cspWhitelist || []).flatMap(d => {
    const trimmed = d.trim()
    const entries = [` https://${trimmed.replace(/^\*\./, '*.').trim()}`]
    if (trimmed.startsWith('*.')) entries.push(` https://${trimmed.slice(2)}`)
    return entries
  }).join('')
  const securityHeaders = `
    include snippets/security-headers.conf;
    add_header Content-Security-Policy "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'${cspExtra}; style-src 'self' 'unsafe-inline'${cspExtra}; img-src 'self' data: https:; font-src 'self' data:${cspExtra}; connect-src 'self'${cspExtra}; frame-src 'self' blob:${cspExtra}; worker-src 'self' blob:; frame-ancestors 'self';" always;`
