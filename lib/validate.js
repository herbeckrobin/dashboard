...
// Custom Headers fuer URL-Fetch Cron-Jobs
const FORBIDDEN_HEADERS = ['cookie', 'host', 'connection', 'transfer-encoding', 'content-length']
const MAX_CRON_HEADERS = 10
const HEADER_KEY_RE = /^[a-zA-Z0-9\-]+$/

export function validateCronHeaders(headers) {
  if (!headers || !Array.isArray(headers)) return { valid: true }
  if (headers.length > MAX_CRON_HEADERS) {
    return { valid: false, error: `Maximal ${MAX_CRON_HEADERS} Custom Headers erlaubt` }
  }
  for (const h of headers) {
    if (!h.key || typeof h.key !== 'string' || !h.key.trim()) {
      return { valid: false, error: 'Header-Key ist erforderlich' }
    }
    if (!HEADER_KEY_RE.test(h.key.trim())) {
      return { valid: false, error: `Ungueltiger Header-Key: ${h.key} (nur Buchstaben, Zahlen, Bindestriche)` }
    }
    if (FORBIDDEN_HEADERS.includes(h.key.trim().toLowerCase())) {
      return { valid: false, error: `Header "${h.key}" ist nicht erlaubt` }
    }
    if (h.value && typeof h.value === 'string' && h.value.length > 2000) {
      return { valid: false, error: `Header-Value fuer "${h.key}" ist zu lang (max 2000 Zeichen)` }
    }
  }
  return { valid: true }
}
