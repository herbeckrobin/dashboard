...
async function lookupAllRecords(domain) {
  // Domain kommt validiert aus der DB — nur Whitelist-Check als Sicherheit
  if (!/^[a-zA-Z0-9.-]+$/.test(domain)) return { a: [], aaaa: [], cname: [], mx: [], txt: [], ns: [] }

  const records = { a: [], aaaa: [], cname: [], mx: [], txt: [], ns: [] }

  // dig ANY ist unzuverlaessig, daher einzelne Typen abfragen
  const result = await runCommand(
    `dig @8.8.8.8 +noall +answer ${domain} A ${domain} AAAA ${domain} CNAME ${domain} MX ${domain} TXT ${domain} NS 2>/dev/null`,
    10000
  )

  if (!result.success || !result.output) {
    records._debug = { success: result.success, output: result.output || null, error: result.error || null }
    return records
  }

  records._debug = { raw: result.output }
