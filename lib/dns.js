...
// Alle DNS-Records einer Domain abfragen (sequentiell um Rate-Limiting zu vermeiden)
async function lookupAllRecords(domain) {
  ...
  try { records.a = await resolver.resolve4(domain) } catch {}
  try { records.aaaa = await resolver.resolve6(domain) } catch {}
  try { records.cname = await resolver.resolveCname(domain) } catch {}
  try {
    records.mx = (await resolver.resolveMx(domain)).sort((a, b) => a.priority - b.priority)
  } catch {}
  try {
    records.txt = (await resolver.resolveTxt(domain)).map(parts => parts.join(''))
  } catch {}
  try { records.ns = await resolver.resolveNs(domain) } catch {}
