...
app.post('/webhook', (req, res) => {
  ...
  console.log('[PUSH]', repoFullName)

  // Projekt anhand Repo-Name finden
  const projects = getProjects()
  const project = projects.find(p => p.repo === repoFullName)

  if (!project) {
    console.log('[SKIP] Kein Projekt fuer', repoFullName)
...
app.listen(PORT, '127.0.0.1', () => {
  console.log(\`[OK] Webhook-Server laeuft auf Port \${PORT} (localhost)\`)
})
`

// Service-Template wird dynamisch mit korrektem Node-Pfad generiert
function getWebhookService(nodePath) {
  return `[Unit]
...
ExecStart=${nodePath} /home/deploy/webhook/webhook-server.js
Restart=always
RestartSec=5
...
export default [
  {
    ...
    async enforce({ runCmd }) {
      ...
      await runCmd('cd /home/deploy/webhook && npm install express@4', { timeout: 30000 })

      // .env mit WEBHOOK_SECRET aus Dashboard (oder generieren)
      ...
      if (!secret) {
        // Fallback: neues Secret generieren
      ...
      // systemd Service (Node-Pfad dynamisch ermitteln)
      const nodePath = (await runCmd('which node')).trim() || '/usr/bin/node'
      const tmpService = '/tmp/webhook-server.service'
      fs.writeFileSync(tmpService, getWebhookService(nodePath))
      ...
      await runCmd('sudo systemctl restart webhook-server')
      // Warten bis Service aktiv
  ...
  {
    id: 'webhook-localhost-binding',
    name: 'Webhook-Server bindet auf 127.0.0.1',
    category: 'bootstrap',
    order: 71,
    scope: 'server',
    severity: 'medium',
    ...
    async audit({ runCmd }) {
      const code = await runCmd('cat /home/deploy/webhook/webhook-server.js 2>/dev/null')
      if (!code) return { passed: true, actual: 'webhook-server.js nicht vorhanden', expected: '127.0.0.1' }
      const bindsLocalhost = code.includes("'127.0.0.1'") || code.includes('"127.0.0.1"')
      const bindsAll = code.includes("'0.0.0.0'") || code.includes('"0.0.0.0"')
      return {
        passed: bindsLocalhost && !bindsAll,
        actual: bindsLocalhost ? '127.0.0.1' : (bindsAll ? '0.0.0.0 (alle Interfaces!)' : 'Binding unklar'),
        expected: '127.0.0.1',
      }
    },

    async enforce({ runCmd }) {
      // 0.0.0.0 durch 127.0.0.1 ersetzen
      await runCmd("sed -i \"s/'0.0.0.0'/'127.0.0.1'/g\" /home/deploy/webhook/webhook-server.js")
      await runCmd('sudo systemctl restart webhook-server')
      await runCmd('sleep 2')
      return { changed: true }
    },
