...
export default [
  {
    ...
    async enforce({ runCmd }) {
      ...
      // Einheitlicher Weg fuer Ubuntu + Debian (kein add-apt-repository noetig)
      ...
      if (repoExists !== 'yes') {
        ...
        if (isUbuntu) {
          // Ubuntu: ppa.launchpadcontent.net
          await runCmd('curl -fsSL https://keyserver.ubuntu.com/pks/lookup?op=get\\&search=0x14aa40ec0831756756d7f66c4f4ea0aae5267a6c | sudo gpg --dearmor -o /usr/share/keyrings/ondrej-php.gpg 2>/dev/null || true')
          await runCmd(`echo "deb [signed-by=/usr/share/keyrings/ondrej-php.gpg] https://ppa.launchpadcontent.net/ondrej/php/ubuntu ${codename} main" | sudo tee /etc/apt/sources.list.d/sury-php.list`)
        } else {
