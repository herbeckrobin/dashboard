...
export default [
  {
    ...
    async enforce({ runCmd }) {
      ...
      // Sury-Repo nur wenn noetig (Debian < 13 oder Ubuntu < 24)
      const needsSury = (isUbuntu && majorVersion < 24) || (!isUbuntu && majorVersion < 13)
      if (needsSury) {
...

  {
    ...
    async audit({ runCmd }) {
      const output = await runCmd('composer --version 2>/dev/null')
...

  {
    ...
    async audit({ runCmd }) {
      const output = await runCmd('wp --info 2>/dev/null')
