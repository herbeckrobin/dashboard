...
const RSPAMD_ACTIONS_CONF = `# Spam-Aktionen — Thresholds
reject = 15;
add_header = 6;
greylist = 4;
`

const RSPAMD_GREYLIST_CONF = `# Greylisting — 5 Min Wartezeit fuer unbekannte Sender
enabled = true;
expire = 86400;
timeout = 300;
key_prefix = "rg";
...
export default [
  {
    ...
    async audit({ runQuick, runCmd }) {
      // Kern-Services die nach diesem Rule laufen muessen
      const coreServices = ['postfix', 'rspamd', 'redis-server']
      const results = {}
      for (const svc of coreServices) {
        results[svc] = (await runQuick(`systemctl is-active ${svc}`)).trim()
      }
      // Dovecot nur pruefen wenn Config schon existiert (kommt bei email-dovecot-config)
      const dovecotConfigured = (await runCmd('test -f /etc/dovecot/conf.d/10-custom.conf && echo yes || echo no')).trim() === 'yes'
      if (dovecotConfigured) {
        results['dovecot'] = (await runQuick('systemctl is-active dovecot')).trim()
      }
      // Pakete installiert pruefen
      const packagesInstalled = (await runCmd('dpkg -l postfix dovecot-imapd rspamd redis-server 2>/dev/null | grep -c "^ii"')).trim()
      const allInstalled = parseInt(packagesInstalled) >= 4

      const allActive = Object.values(results).every(s => s === 'active')
      const status = Object.entries(results).map(([k, v]) => `${k}: ${v}`).join(', ')
      return {
        passed: allInstalled && allActive,
        actual: allInstalled ? status : 'Pakete nicht installiert',
        expected: 'E-Mail Pakete installiert und Services aktiv',
      }
    },
    ...
    async enforce({ runCmd }) {
      ...
      // Services starten — Redis zuerst (rspamd braucht Redis)
      // Dovecot wird hier nur enabled, nicht gestartet (Config fehlt noch, kommt bei email-dovecot-config)
      for (const svc of ['redis-server', 'postfix', 'rspamd']) {
        await runCmd(`sudo systemctl enable ${svc}`)
        await runCmd(`sudo systemctl start ${svc}`)
      }
      await runCmd('sudo systemctl enable dovecot')
      // Dovecot starten nur wenn Config schon existiert
      const hasConfig = (await runCmd('test -f /etc/dovecot/conf.d/10-custom.conf && echo yes || echo no')).trim()
      if (hasConfig === 'yes') {
        await runCmd('sudo systemctl start dovecot')
      }
      return { changed: true }
    },
  },

  {
    ...
    async audit({ runCmd, getConfig }) {
      ...
      const myhostname = (await runCmd('/usr/sbin/postconf -h myhostname 2>/dev/null')).trim()
      const transport = (await runCmd('/usr/sbin/postconf -h virtual_transport 2>/dev/null')).trim()
      const masterCf = await runCmd('grep -c "^submission" /etc/postfix/master.cf 2>/dev/null')

      const issues = []
      if (myhostname.includes('Command failed') || !myhostname) issues.push('postconf nicht erreichbar')
      else if (myhostname !== hostname) issues.push(`myhostname: ${myhostname}`)
      if (transport.includes('Command failed') || !transport) issues.push('postconf nicht erreichbar')
      else if (!transport.includes('lmtp')) issues.push(`transport: ${transport}`)
...

  {
    ...
    async enforce({ runCmd }) {
      ...
      // Dovecot (neu)starten — jetzt erst ist die Config komplett
      await runCmd('sudo systemctl enable dovecot')
  ...
  {
    id: 'email-rspamd-config',
    name: 'rspamd korrekt konfiguriert (Spam-Thresholds, Greylisting)',
    category: 'bootstrap',
    order: 63,
    scope: 'server',
    severity: 'medium',
    ...
    async audit({ runCmd }) {
      const actionsConf = (await runCmd('test -f /etc/rspamd/local.d/actions.conf && echo yes || echo no')).trim()
      const greylistConf = (await runCmd('test -f /etc/rspamd/local.d/greylist.conf && echo yes || echo no')).trim()

      const issues = []
      if (actionsConf !== 'yes') issues.push('actions.conf fehlt')
      if (greylistConf !== 'yes') issues.push('greylist.conf fehlt')

      return {
        passed: issues.length === 0,
        actual: issues.length ? issues.join(', ') : 'Konfiguriert',
        expected: 'rspamd actions.conf + greylist.conf vorhanden',
      }
    },
    ...
    async enforce({ runCmd }) {
      ...
      await runCmd('sudo mkdir -p /etc/rspamd/local.d')

      // actions.conf
      const tmpActions = '/tmp/rspamd-actions.conf'
      writeFileSync(tmpActions, RSPAMD_ACTIONS_CONF)
      await runCmd(`sudo mv ${tmpActions} /etc/rspamd/local.d/actions.conf`)

      // greylist.conf
      const tmpGreylist = '/tmp/rspamd-greylist.conf'
      writeFileSync(tmpGreylist, RSPAMD_GREYLIST_CONF)
      await runCmd(`sudo mv ${tmpGreylist} /etc/rspamd/local.d/greylist.conf`)

      await runCmd('sudo systemctl restart rspamd')
      return { changed: true }
    },
  },
]
