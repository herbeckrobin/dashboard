...
export default [
...

  {
    ...
    async audit({ runCmd }) {
      ...
      // Pruefen ob ungeschuetztes 3000:3000 vorkommt (mit oder ohne Quotes)
      const hasOpen = /[- ] *["']?3000:3000["']?/.test(compose) && !hasLocalhost
      return {
    ...
    async enforce({ runCmd }) {
      // Port-Binding in docker-compose.yml auf 127.0.0.1 aendern (quoted + unquoted)
