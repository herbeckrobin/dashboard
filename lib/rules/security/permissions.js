...
export default [
...

  {
    ...
    async audit({ runCmd }) {
      const perms = (await runCmd('stat -c "%a" /home/deploy/dashboard/data 2>/dev/null')).trim()
      return {
        passed: perms === '700',
    ...
    async enforce({ runCmd }) {
      await runCmd('chmod 700 /home/deploy/dashboard/data')
      return { changed: true }
    },
  },

  {
    ...
    async audit({ runCmd }) {
      const perms = (await runCmd('stat -c "%a" /home/deploy/dashboard/data/auth.json 2>/dev/null')).trim()
    ...
    async enforce({ runCmd }) {
      await runCmd('chmod 600 /home/deploy/dashboard/data/auth.json')
      return { changed: true }
    },
  },

  {
    ...
    async audit({ runCmd }) {
      const perms = (await runCmd('stat -c "%a" /home/deploy/dashboard/data/config.json 2>/dev/null')).trim()
      if (!perms) return { passed: true, actual: 'Datei nicht vorhanden', expected: '600' }
      return {
        passed: perms === '600',
        actual: perms,
    ...
    async enforce({ runCmd }) {
      await runCmd('chmod 600 /home/deploy/dashboard/data/config.json')
      return { changed: true }
    },
  },

  {
    ...
    async audit({ runCmd }) {
      const perms = (await runCmd('stat -c "%a" /home/deploy/dashboard/data/projects.json 2>/dev/null')).trim()
      if (!perms) return { passed: true, actual: 'Datei nicht vorhanden', expected: '600' }
      return {
        passed: perms === '600',
        actual: perms,
    ...
    async enforce({ runCmd }) {
      await runCmd('chmod 600 /home/deploy/dashboard/data/projects.json')
      return { changed: true }
    },
  },
