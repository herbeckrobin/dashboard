...
export default [
...

  {
    ...
    async enforce({ runCmd }) {
      ...
      // Kaputte literal \n Eintraege von fehlerhaftem echo bereinigen
      await runCmd("sudo sed -i '/^.*\\\\n\\[dovecot\\]/d' /etc/fail2ban/jail.local 2>/dev/null || true")
      const jailConfig = await runCmd('cat /etc/fail2ban/jail.local 2>/dev/null')
...

  {
    ...
    async enforce({ runCmd }) {
      await ensureJailLocal(runCmd)
      // Kaputte literal \n Eintraege von fehlerhaftem echo bereinigen
      await runCmd("sudo sed -i '/^.*\\\\n\\[postfix\\]/d' /etc/fail2ban/jail.local 2>/dev/null || true")
      const jailConfig = await runCmd('cat /etc/fail2ban/jail.local 2>/dev/null')
