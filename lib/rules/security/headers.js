...
export default [
  {
    ...
    async audit({ project, runCmd }) {
      ...
      if (!config) return { passed: false, actual: 'nginx Config fehlt', expected: 'HSTS Header mit preload' }
      const hasHSTS = config.includes('Strict-Transport-Security')
      const hasPreload = config.includes('preload')
      return {
        passed: hasHSTS && hasPreload,
        actual: hasHSTS ? (hasPreload ? 'HSTS mit preload' : 'HSTS ohne preload') : 'Fehlt',
        expected: 'HSTS Header mit preload',
      }
    },
    enforce: null, // Wird ueber nginx-config Rule in infra/ behoben (Re-Deploy noetig)
  },
