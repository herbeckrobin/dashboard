...
export default [
...

  {
    ...
    async audit({ project, runCmd }) {
      ...
      // Snippet-Include zaehlt als alle Standard-Headers vorhanden
      if (config.includes('snippets/security-headers.conf')) {
        return { passed: true, actual: 'Alle vorhanden (via Snippet)', expected: 'Alle 5 Security Headers' }
      }
